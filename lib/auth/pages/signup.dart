...
import 'package:session2d2v1/auth/pages/home.dart';
import 'package:session2d2v1/auth/widgets/customtextfield.dart';
import 'package:session2d2v1/auth/pages/signin.dart';
import 'package:supabase_flutter/supabase_flutter.dart';
import 'package:session2d2v1/main.dart';
import 'home.dart';
              ...
              Column(
                ...
                children: [
                  ...
                  SizedBox(
                      ...
                      child: CustomTextField(onChange: (str){setState(() {
                  ...
                  SizedBox(width: MediaQuery.of(context).size.width - 48,
                    height: 46,
                    child: FilledButton(onPressed: (isValid() == true)?
                    ()async {
                      try{
                      var res = await supabase.auth.signUp(password: passc.text, email: emailc.text);
                      var user = res.user;
                      var session = res.session;
                      if(user != null){await supabase.from('profiles').insert(
                        {'id_user':user.id,
                        'phone' : phonec.text,
                          'name' : namec.text,
                          'avatar' : ''
